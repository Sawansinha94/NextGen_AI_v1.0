...
def canonicalize_faq_query(normalized_query: str) -> str:
    """Map common incident-status wording to the seeded FAQ phrasing."""
    compact_query = re.sub(r"\s+", " ", normalized_query).strip().lower()
    if re.search(r"\b(status|state)\b", compact_query) and "incxxxxxxx" in compact_query:
        return "what is the status of INCXXXXXXX?"
...
def main() -> int:
    ...
    try:
        ...
        log_operation(
            connection,
            args.user_name,
            f"NORMALIZE faq_snow_query.main: query={normalized_query}; faq_query={faq_query}; incident_count={len(incident_numbers)}",
            db_settings.logging_table,
        )
        faq_rows = search_faq(connection, db_settings, faq_query, 1, args.user_name)
        if not faq_rows:
